...
public abstract class PropertyAccessorUtils {
    ...
    public static boolean matchesProperty(String registeredPath, String propertyPath) {
        ...
        return (registeredPath.indexOf(PropertyAccessor.PROPERTY_KEY_PREFIX_CHAR, propertyPath.length() + 1) ==
    ...
    public static String canonicalPropertyName(@Nullable String propertyName) {
        ...
        while (searchIndex != -1) {
            ...
            searchIndex = -1;
            if (keyStart != -1) {
                int ketEnd = sb.indexOf(
                        PropertyAccessor.PROPERTY_KEY_SUFFIX, keyStart + PropertyAccessor.PROPERTY_KEY_PREFIX.length());
                if (ketEnd != -1) {
                    String key = sb.substring(keyStart + PropertyAccessor.PROPERTY_KEY_PREFIX.length(), ketEnd);
                    if ((key.startsWith("'") && key.endsWith("'") || (key.startsWith("\"")) && key.endsWith("\""))) {
                        sb.delete(keyStart + 1, keyStart + 2);
                        sb.delete(ketEnd - 2, ketEnd - 1);
                        ketEnd = ketEnd - 1;
                    }
                    searchIndex = ketEnd + PropertyAccessor.PROPERTY_KEY_PREFIX.length();
                }
            }
        }
        return sb.toString();
