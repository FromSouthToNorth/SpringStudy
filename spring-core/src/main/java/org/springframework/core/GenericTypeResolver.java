...
public final class GenericTypeResolver {
    ...
    private static Class<?> getSingleGeneric(ResolvableType resolvableType) {
        Assert.isTrue(resolvableType.getGenerics().length == 1,
                () -> "Expected 1 type argument no generic interface [" + resolvableType +
    ...
    public static Type resolveType(Type genericType, @Nullable Class<?> contextClass) {
        if (contextClass != null) {
            ...
            ResolvableType resolvableType = ResolvableType.forType(genericType);
            if (resolvableType.hasUnresolvableGenerics()) {
                ...
                for (int i = 0; i < typeArguments.length; i++) {
                    ...
                    if (typeArgument instanceof TypeVariable) {
                        ResolvableType resolvableTypeArgument = resolveVariable(
                                (TypeVariable<?>) typeArgument, contextType);
                        if (resolvableTypeArgument != ResolvableType.NONE) {
                            generics[i] = resolvableTypeArgument.resolve();
                        }
                        else {
    ...
    private static void buildTypeVariableMap(ResolvableType type, Map<TypeVariable, Type> typeVariableMap) {
        if (type != ResolvableType.NONE) {
            ...
            if (resolve != null && type.getType() instanceof ParameterizedType) {
