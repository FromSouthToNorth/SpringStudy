...
public class MethodParameter {
     ...
     * @since 5.2
     */
    private MethodParameter nested(@Nullable Integer typeIndex) {
    ...
    private boolean hasNullableAnnotation() {
        for (Annotation ann : getMethodAnnotations()) {
        ...
        if (paramType == null) {
            if (this.parameterIndex < 0) {
                ...
                if (this.executable instanceof Constructor &&
                    ...
                    // javac 中的错误：类型数组不包括封闭的实例参数
                    // 对于具有至少一个泛型构造函数参数的内部类，
                    // 所以用实际参数索引减1来访问它
    ...
    public Annotation[] getParameterAnnotations() {
        ...
        if (paramAnns == null) {
            ...
            if (this.executable instanceof Constructor &&
                ...
                // JDK <9 中的 javac 中的错误：注释数组不包括封闭的实例参数
                // 对于内部类，因此访问它时将实际参数索引降低 1
                index = this.parameterIndex - 1;
            }
    ...
    public boolean equals(@Nullable Object other) {
        ...
        return (getDeclaringClass() == otherParam.getContainingClass() &&
    ...
    public static MethodParameter forExecutable(Executable executable, int parameterIndex) {
        if (executable instanceof Method) {
            ...
            throw new IllegalStateException("Not a Method/Constructor: " + executable);
        }
    }
    ...
    protected static int findParameterIndex(Parameter parameter) {
        ...
        // 可能会再次尝试使用对象相等性检查以避免竞争
        // 调用 java.lang.reflect.Executable.getParameters() 时的条件。
        for (int i = 0; i < allParams.length; i++) {
