...
public class AntPathMatcher implements PathMatcher {
    ...
    private final Map<String, String[]> tokenizedPattenCache = new ConcurrentHashMap<>(256);
    ...
    private void deactivatePatternCache() {
        ...
        this.tokenizedPattenCache.clear();
    ...
    protected boolean doMatch(String pattern, @Nullable String path, boolean fullMatch,
            @Nullable Map<String, String> utiTemplateVariables) {
        ...
        int pathIdxStart = 0;
        int pathIdxEnd = pattDirs.length - 1;
        ...
        while (pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
            ...
            if (!matchString(pattDir, pattDirs[pathIdxStart], utiTemplateVariables)) {
                return false;
            }
            pattIdxStart++;
            pathIdxStart++;
        ...
        while (pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
            ...
            if (!matchString(pattDir, pathDirs[pathIdxEnd], utiTemplateVariables)) {
                return false;
            }
            pattIdxEnd--;
            pathIdxEnd--;
        ...
        while (pattIdxStart != pattIdxEnd && pathIdxStart <= pathIdxEnd) {
            ...
            for (int i = 0; i <= strLength - patLength; i++) {
                for (int j = 0; j < patLength; j++) {
                    ...
                    if (!matchString(subPat, subStr, utiTemplateVariables)) {
                        continue strLoop;
                    }
                }
    ...
    protected String[] tokenizePattern(String pattern) {
        ...
        if (cachePatterns == null || cachePatterns.booleanValue()) {
            tokenized = this.tokenizedPattenCache.get(pattern);
        }
        if (tokenized == null) {
            tokenized = tokenizePath(pattern);
            if (cachePatterns == null && this.tokenizedPattenCache.size() >= CACHE_TURNOFF_THRESHOLD) {
                deactivatePatternCache();
                return tokenized;
            }
            if (cachePatterns == null || cachePatterns.booleanValue()) {
                this.tokenizedPattenCache.put(pattern, tokenized);
            }
        }
        return tokenized;
    }
     ...
     */
    private boolean matchString(String pattern, String str,
            @Nullable Map<String, String> uriTemplateVariables) {

        return getStringMatcher(pattern).matchString(str, uriTemplateVariables);
    }
    ...
    protected static class AntPathStringMatcher {
         ...
         * @return {@code true} if the string matches against the pattern, or {@code false} otherwise.
         */
        public boolean matchString(String str, @Nullable Map<String, String> uriTemplateVariables) {
            if (this.exactMatch) {
